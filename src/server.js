...
if (process.env.NODE_ENV) process.env.NODE_ENV = process.env.NODE_ENV.toLowerCase();
...
app.use(helmet({
  ...
  contentSecurityPolicy: {
    ...
    directives: {
      defaultSrc: ["'self'"],
      scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https://apis.google.com", "https://cdn.tailwindcss.com"],
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (allowedOrigins.length === 0) {
      console.error('⚠️  CORS_ORIGIN is not set in production — all cross-origin requests blocked.');
      return callback(new Error('CORS not configured. Contact the server administrator.'));
    }
  ...
  allowedHeaders: ['Origin', 'X-Requested-With', 'Content-Type', 'Accept', 'Authorization', 'Content-Length', 'lang'],
  credentials: true,
